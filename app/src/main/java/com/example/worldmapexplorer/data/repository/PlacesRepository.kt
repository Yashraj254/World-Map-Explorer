...
class PlacesRepository @Inject constructor(
    ...
    suspend fun fetchPlaces(query: String, excludedPaces: String) = nominatimClient.fetchPlaces(query, excludedPaces)
    ...
    suspend fun fetchPrefix(result: Map<String, String>): String {
        ...
        if (result["boundary"] == "administrative" && result.containsKey("admin_level")) {
            ...
            for ((key, value) in result) {
                if (prefixes.has(key)) {
                    val formattedValue = value.replaceFirstChar { it.uppercaseChar() }.replace("_", " ")
    ...
    suspend fun getRoute(locations: List<LatLon>): ApiResponse<RouteResponseDto>{
        val routeRequestDto = RouteRequestDto(locations, "auto", RouteRequestDto.DirectionsOptions("km"))
